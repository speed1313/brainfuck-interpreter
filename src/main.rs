...
enum Instruction {
    ...
    Write,
    Read,
    Loop(Vec<Instruction>), // Loop contains its inner instructions.
}
...
fn parse(opcodes: Vec<OpCode>) -> Vec<Instruction> {
    ...
    for (i, op) in opcodes.iter().enumerate() {
        if loop_stack == 0 {
            ...
            // if instr is instruction, then push it to program stack
            match instr {
                Some(instr) => program.push(instr),
                None => (), // if loop instruction, dont push it.
            }
        } else {
            // in loop
            match op {
                ...
                OpCode::LoopEnd => {
                    ...
                    if loop_stack == 0 {
                        // if loop_stack is clear, then push it. parse inner loop program recursively.
